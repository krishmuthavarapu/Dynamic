...
<head>
  ...
  $sqlPages = mysqli_query($db_conn, "SELECT id, title, team1, team2, thumbnail, thumbnail1, content, active, datetime FROM pages WHERE active=1 ORDER BY datetime DESC"); //uses while loop

  $sqlPagesActive = mysqli_query($db_conn, "SELECT id, title, team1, team2, thumbnail, thumbnail1, content, active FROM pages WHERE active=1"); //uses while loop
  ?>
  <style>
    .logo-pop {
      width: 160px;
...
<body style="background-image: url('img/ground5.jpg'); background-repeat: no-repeat; background-size: cover;">
  ...
  <section class="mask rgba-black-light overflow-hidden sect" style=" background-color: rgba(0,0,0,0.5);">
    ...
    padding: 5px;">FREE PYTHON COACHING</b></strong>
          </p>
